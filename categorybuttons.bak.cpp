...
void CategoryButtons::colorChanged(QString color, int id, bool categoryFlag)
{
    QSqlQuery query;
    if (categoryFlag) {
        // This was a category button -> update the category table!
        if (query.exec("UPDATE category SET color = '" + color + "' WHERE id = " + QString::number(id) + ";")) {
            qDebug() << "chagned color";
        } else {
            qDebug() << "error changing color";
            qDebug() << query.lastError().text();
        }
    } else {
        // This comes from an article button
        query.exec("UPDATE article SET color = '" + color + "' WHERE plu = " + QString::number(id) + ";");
    }
